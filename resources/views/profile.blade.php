                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-6">
                            <div class="form-group">
                                <a href="{{route('main.show', auth()->user()->id)}}" class="btn  btn-light">Посмотреть профиль</a>
                            </div>
                            <br>
                            <div class="form-group">
                                <label>Прямая ссылка на ваш профиль</label>
                                <input type="text" class="form-control" placeholder="Текст ..." value="{{route('main.show', auth()->user()->id)}}" disabled>
                            </div>
                            <br>
                            <div class="form-group">
                                <label>Тип отображения профиля</label>
                                <select name="type" id="" required class="form-control">
                                    <option {{auth()->user()->type == 'standard' ? 'selected' : ''}} value="standard">Стандартный</option>
                                    <option {{auth()->user()->type == 'contacts' ? 'selected' : ''}} value="contacts">Контакты</option>
                                </select>
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        ...
                        <div class="col-sm-6">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" required name="name" placeholder="Текст ..." value="{{auth()->user()->name}}">
                            </div>
                            <br>
                            <div class="form-group">
                                <label>Телефон</label>
                                <input type="tel" name="phone" class="form-control" placeholder="Номер ..." value="{{auth()->user()->phone}}">
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-sm-12">
                            ...
                            <textarea placeholder="Текст ..." class="form-control" name="information" id=""  cols="30" rows="10">{{auth()->user()->information}}</textarea>
                        </div>
                    </div>
                   <div class="row">
